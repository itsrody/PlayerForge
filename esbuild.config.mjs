...
// Instant-injection note: @run-at document-start is honored natively by
// Violentmonkey 2.49+ via the WebExtension UserScripts API. PF never assumes
// the DOM is ready at eval, so it is safe under instant injection.
const banner = `// ==UserScript==
// @name         PlayerForge (Firefox)
// @namespace    https://github.com/PlayerForge
// @version      0.7.2
// @description  Firefox Beta 157+ / Violentmonkey 2.49+ exclusive HTML5 video player enhancer with gestures, hotkeys, progress resume, subtitles, and an extensible plugin system
// @author       PlayerForge
 ...
 * those promises - or silently corrupt the metadata block Violentmonkey reads
...
if (watch) {
  ...
  // Primary: minified bundle (what Violentmonkey installs).
