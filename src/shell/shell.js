...
export class Shell {
  ...
  /**
   * Unified contextual reference box: in inline mode the reference is the
   * shell's own container (the SDK container). Fullscreen reference box used
   * for fill-mode cover scaling and scrub normalization; on Firefox the
   * fullscreen element covers the full screen with no safe-area involvement,
   * so screen.width/height is the correct geometry. Returns { width, height }.
   */
  get referenceBox() {
    if (fs) {
