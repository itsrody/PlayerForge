...
test("referenceBox in fullscreen is the physical screen", async () => {
  ...
  // Firefox's fullscreen iframe fills the whole display (the top document's
  // viewport-fit=cover drives cutout edge-to-edge), so the reference is
  // screen.* - no env-based safe-rect narrowing applies inside the iframe.
  setFullscreen(dom, container);
